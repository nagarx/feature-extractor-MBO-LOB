...
fn test_depth_ticks_requires_correct_tick_size_configuration() {
    ...
    let mut aggregator_correct = MboAggregator::new().with_tick_size(0.001); // Correct $0.001 tick for crypto
...
fn test_depth_ticks_with_configured_tick_size() {
    ...
    let mut aggregator = MboAggregator::new().with_tick_size(crypto_tick_dollars);
...
fn test_feature_extractor_uses_config_tick_size() {
    ...
    for i in 0..100 {
        let event = MboEvent::new(i * 1_000_000, Action::Add, Side::Bid, best_bid, 100, i);
...
fn test_different_tick_sizes() {
    ...
    for (tick_dollars, tick_nano, description) in test_cases {
        ...
        let mut aggregator = MboAggregator::new().with_tick_size(tick_dollars);
